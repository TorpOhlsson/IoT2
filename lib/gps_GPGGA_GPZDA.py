# GPS library, for other frames than $GPGGA, $GPZDA and general GNSS modify the parser
# Developed on u-blox, milliseconds are not parsed


class GPS_GPGGA_GPZDA:
    ...
    __satellites = 0
    __hdop = 0.0
    __GPGGA_received = False # tjek om den er True i test programmet
    __GPZDA_received = False # tjek om den er True i test programmet
    ...
    def __init__(self, uart, all_nmea = False):
        ...
        if self.all_nmea == True:
            ...
            uart.write("$PUBX,40,GLL,0,0,0,0*5C\n") # Disable all but the $GPGGA and $GPZDA frames
            ...
            uart.write("$PUBX,40,RMC,0,0,0,0*47\n")
    ...
    def __parse_nmea_frame(self, string):   # Change to parse all relevant frames: http://aprs.gids.nl/nmea/, no checksum validation
        ...
        # Parse $GPZDA
            ...
            # UTC year, month and day may be omitted here since it can be parsed from the $GPGGA frame
            # UTC day
            if len(subFrame[2]) > 0:
    ...
    def get_test_frames(self):
        return self.__GPZDA_received, self.__GPGGA_received
